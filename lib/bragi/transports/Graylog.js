...
function TransportGraylog ( options ){
    ...
    this.fromHostname = options.fromHostname || os.hostname();
...
TransportGraylog.prototype.log = function transportGraylogLog( loggedObject ){
    ...
    // setup message
	dataToSend.short_message = loggedObject.originalMessage;
...
TransportGraylog.prototype.sendChunked = function sendChunked(graylogClient, compressedMessage) {
	...
	var messageId = ''+(new Date() + (Math.random()*100000 | 0));
    ...
    if (sequenceSize > 128) {
        console.log("Error: sequence size it too large");
        return false;
    }
	...
	for( sequence=0; sequence < sequenceSize; sequence++ ) {
        chunkBytes = (byteOffset + this.chunkSize) < compressedMessage.length ? 
            this.chunkSize : (compressedMessage.length - byteOffset);
