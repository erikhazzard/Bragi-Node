...
Transports.prototype.get = function get( transportName ){
    ...
    returnedTransportObjects.property = function getSetProperty( key, value ){
        // Allow `.property()` to be called, which will return an array of 
        // values if just the `key` is passed in. If `value` is also passed in,
        // it will set all returned transports's key to that value
