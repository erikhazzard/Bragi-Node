...
function deepRemoveWhitelistBlacklistKeys (key, targetObject) {
    // prepare whitelist / blacklist
    let hasWhitelist = LOGGER.options.keyWhitelist;
    let hasBlacklist = LOGGER.options.keyBlacklist;
    ...
    if (canLogKey && targetValue && typeof targetValue === 'object') {
        ...
        for (var i = 0; i < innerKeys.length; i++) {
            subKey = innerKeys[i];
            deepRemoveWhitelistBlacklistKeys(subKey, targetValue);
        }
