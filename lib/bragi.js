 ...
 *
 * For hiding unformatted parameters (e.g., if we want to by default NOT store
 *      additional parameters in the messages, for example in 
 *      `logger.log('group', 'message', {a:1}, {b:1});`
 *      pass in `hideUnformattedParameters` as `true`
 *
 *      LOGGER.options.hideUnformattedParameters = false (by default)
 *
 * TRANSPORTS
...
LOGGER.options = {
    ...
    // Whether additional parameters should be injected into the message string
    // This is done in the library core instead of at the transport level,
    // for now, as if affects the message itself that all transports receive
    hideUnformattedParameters: false,

    // Whitelist of allowed keys in objects to be logged. e.g.
    // May be an object of key names, like {keyName: true, keyName2: true}
    // or `null` or `true` for everything
    keyWhitelist: null,

    // Blacklist of keys to ignore. This takes higher priority over whitelist.
    // May be an object of key names, like {keyName: false, keyName2: false}
    // Can be `false` or `null` (no blacklist) or an array of key names
    keyBlacklist: null,

    // Default value for fields that are hidden by white / black list
    hiddenFieldValue: '--- HIDDEN ---',
};
...
LOGGER.removeGroup = function removeGroup ( group ){
    // Takes in a group and removes all occurrences of it from
    // groupsEnabled
...
//
// Util for removing properties in .log() call
function deepRemoveWhitelistBlacklistKeys (key, targetObject, _callSize) {
    // limit number of recursive calls
    if (_callSize === undefined) { _callSize = -1; }
    _callSize++;

    // if the stack has gotten too deep, remove log
    if (_callSize > 5) {
        targetObject[key] = LOGGER.options.hiddenFieldValue;
        return false;
    }

    // prepare whitelist / blacklist
    var hasWhitelist = LOGGER.options.keyWhitelist;
    var hasBlacklist = LOGGER.options.keyBlacklist;
    ...
    // check whitelist
    if (hasWhitelist) {
        // can NOT log by default if there's a whitelist
        canLogKey = false;

        // can log if it's in the whitelist
        if (
            LOGGER.options.keyWhitelist === true ||
            LOGGER.options.keyWhitelist[key]
        ) {
            canLogKey = true;
        }
    }
    ...
    if (
        hasBlacklist &&
        LOGGER.options.keyBlacklist[key]
    ) {
        canLogKey = false;
    }
    ...
    if (canLogKey && targetValue && typeof targetValue === 'object') {
        // check all inner keys
        var innerKeys = Object.keys(targetValue);
        var subKey;
        // If there are too many keys, don't store them all
        var keysLength = Math.min(innerKeys.length, 300);

        for (var i = 0; i < keysLength; i++) {
            subKey = innerKeys[i];
            deepRemoveWhitelistBlacklistKeys(subKey, targetValue, _callSize);
        }

        return canLogKey;
    } else {
        // not an object, do a check
        return canLogKey;
    }
}

// Regex for formatter for printing args
var FORMATTER_REGEX = /%([Jj]|[Ss]|[Dd]|[Oo])/g;
...
LOGGER.log = function loggerLog(group, message){
    ...
    extraArgs = _.cloneDeep(extraArgs);
    ...
    // turn white and blacklists into objects if they are arrays
    if (LOGGER.options.keyWhitelist instanceof Array) {
        var whitelistObject = {};
        for (var i = 0; i < keyWhitelist.length; i++) {
            whitelistObject = keyWhitelist[i];
        }
        LOGGER.options.keyWhitelist = whitelistObject;
    }
    if (LOGGER.options.keyBlacklist instanceof Array) {
        var blacklistObject = {};
        for (var i = 0; i < keyBlacklist.length; i++) {
            blacklistObject = keyBlacklist[i];
        }
        LOGGER.options.keyBlacklist = blacklistObject;
    }
    ...
    for(i = 0; i < extraArgs.length; i++){
        ...
        if(!(extraArgs[i] instanceof Array) && typeof extraArgs[i] === 'object'){
            for(var key in extraArgs[i]){
                // check items
                var canLogKey = deepRemoveWhitelistBlacklistKeys(key, extraArgs[i]);

                // Property *can* be logged
                if (canLogKey) {
                    loggedObject.properties[key] = extraArgs[i][key];
                } else {
                    extraArgs[i][key] = LOGGER.options.hiddenFieldValue;
                    loggedObject.properties[key] = LOGGER.options.hiddenFieldValue;
                }
            }
        } else {
    ...
    loggedObject.unixTimestamp = newDate.getTime() / 1000;
    ...
    var numOfFormatters = ('' + (message || '')).match(FORMATTER_REGEX);
    numOfFormatters = numOfFormatters ? numOfFormatters.length : 0;

    // store the shortened value in case callers don't use the full `Parameters`
    // key
    for(var tmpArg in extraArgs){
        tmpVal = extraArgs[tmpArg];
        if(tmpVal === undefined){ tmpVal = 'undefined'; }

        if(tmpVal && !tmpVal._hide && true){
            if(LOGGER.options.hideUnformattedParameters === true ||
                LOGGER.options.hideUnformattedParams === true
              ){
                if(utilFormatArgs.length < numOfFormatters){
                    // Only push if the formatter exists
                    utilFormatArgs.push(extraArgs[tmpArg]);
                }
            } else {
                // Otherwise, always push it
                utilFormatArgs.push(extraArgs[tmpArg]);
            }
        }
    }
