 ...
 *      LOGGER.options.groupsEnabled = true; // Shows ALL messages (false to show none)
 ...
 *      somewhere if you want to access the stored messages
 *
...
LOGGER.options = {
    ...
    // History
    // ----------------------------------
    // storeHistory: {Boolean} specifies whether to save all log message 
    ...
    //      store history for messages found in groupsEnabled. 
...
// --------------------------------------
for(var i=0; i<LOGGER.options.transports.length; i++){
    LOGGER.transports.add( LOGGER.options.transports[i] );
}
...
LOGGER.log = function loggerLog(group, message){
    ...
    if( !canLog(group, LOGGER.options.groupsEnabled, LOGGER.options.groupsDisabled) ){ 
    ...
    // ----------------------------------
    var groupsEnabled, curTransport;
    ...
    for(var transport in LOGGER.transports.transports){
        ...
        if(curTransport.groupsEnabled !== undefined){
            groupsEnabled = curTransport.groupsEnabled;
        }
        if(curTransport.groupsDisabled !== undefined){
            groupsDisabled = curTransport.groupsDisabled;
        }

        // check if message can be logged
        if(canLog(loggedObject.group, groupsEnabled, groupsDisabled)){
            curTransport.log( 
